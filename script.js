...
async function sendQuizResult() {
    const resultData = { language: currentLanguage, correctAnswers: scoreCount, totalQuestions: questionsArray.length, date: new Date().toISOString() };
    await fetch('https://jsonplaceholder.typicode.com/posts', { method: 'POST', body: JSON.stringify(resultData), headers: { 'Content-Type': 'application/json' } });
}
...
window.addEventListener('load', () => { if (loadState()) renderQuestion(); });
